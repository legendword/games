const { allowOrigin, cards } = require('./constants');
const games = require('./games');
const game = games.landlord;
        ...
        if (socket.usertoken && players[socket.usertoken]) {
            ...
            if (rooms[rid].gameEnded) {
                if (rooms[rid].playAgain === false) {
                    socket.emit('notify', {
                        color: 'negative',
                        message: 'A player has already left the room.'
                    })
                    return
                }
                if (rooms[rid].playAgain.includes(socket.usertoken)) {
                    ...
                    if (rooms[rid].playAgain.length == 3) {
    ...
    socket.on('move', (obj) => {
        ...
        if (room.roundId == 0) { //calling landlord round
            ...
            if (obj.value < 0 || obj.value > 3) return
                ...
                roomGames[rid].hands[playerPos] = roomGames[rid].hands[playerPos].sort((a,b) => realCardValue(cards[b].value)-realCardValue(cards[a].value))
                ...
                if (rooms[rid].roundCounter == 2) { //2 players have chosen a number, the 3rd player chose a number other than 3, so everyone chose a number other than 3
                    ...
                    //don't forget to sort and update cardCount
                    roomGames[rid].hands[maxNumPos] = roomGames[rid].hands[maxNumPos].sort((a,b) => realCardValue(cards[b].value)-realCardValue(cards[a].value))
            ...
            if ((room.lastMove == -1 || room.lastMove == playerPos) && obj.type == 'pass') {
                socket.emit('notify', {
                    color: 'negative',
            ...
            if (obj.type == 'pass') {
                ...
                let playedCards = obj.value.sort((a,b) => realCardValue(cards[b].value)-realCardValue(cards[a].value))
                ...
                if (canBePlayed(playedCards, room.currentCards)) {
                    ...
                    //v1.0.1 -> v1.1: lastMove no longer is 'card', but the actual cards played.
                    rooms[rid].players[playerPos].lastMove = playedCards
    ...
    if (maxSameValue == 1) {
        ...
        if (ptCards.length == 3) return [3, cardValues[0]] //3 same cards
            ...
            if (numbers[1] == 0 && numbers[2] == 0) { //pure ladder of 3s
                ...
                if (cardValues[cur-1] >= 15) return -1 //ladder cannot include anything greater than or equal to '2' (realValue 15)
...
function roomCheckStart(roomId) {
    ...
    if ((room.gameState == 'lobby' && room.playerCount == room.playerLimit) || (room.gameState == 'game' && room.gameEnded && room.playAgain && room.playAgain.length == room.playerLimit)) {
        ...
        for (let i of room.players) {
            if (i.state != 'ready' && i.state != 'game') return //add game state for playAgain
        }
        //start the game
        //initial cards (randomly give 17 cards each to 3 players, and leave 3)
        let pc = [[], [], [], []]
        let cardIds = Object.keys(cards)
        if (cardIds.length !== 54) console.error('Cards length is not 54!')
        for (let i=0;i<3;i++) {
            for (let j=0;j<17;j++) {
                let k = Math.floor(Math.random()*cardIds.length)
                pc[i].push(cardIds[k])
                cardIds.splice(k, 1)
            }
        }
        if (cardIds.length !== 3) console.error('Cards left is not 3!')
        pc[3] = [...cardIds]
        for (let i=0;i<4;i++) { //sort from biggest to lowest value
            pc[i] = pc[i].sort((a,b) => realCardValue(cards[b].value)-realCardValue(cards[a].value))
        }
        ...
        for (let i=0;i<room.players.length;i++) { //init player
            ...
            rooms[roomId].players[i].lastMove = ''
        }
